...
FILE *settings;
int CPS = 10, timeout_clicks = 45;
int close_key = 77, toggle_key = 78;
char file_pos[256] = "tsclick.conf";
...
void RunClickerA() {
    ...
    printf("[WARNING] Press Right CTRL to change settings..\n");
    printf("TSClicker (Engine) 1.0.0a\n\n");

    bool running = 1, auth = 1;
    ...
    while (running) {
        ...
        if (GetAsyncKeyState(close_key) != 0 && auth) running = 0;
        if (GetAsyncKeyState(toggle_key) != 0) mouse_click();
    }
...
void create_config() {
    ...
    printf("Press the closing key >> \n");
    Sleep(1000);
    close_key = reg_key();

    printf("Press the toggling key >> ");
    Sleep(1000);
    toggle_key = reg_key();
    ...
    printf("Wait 1 second and the clicker will be running with the new settings");
    Sleep(1000);
...
void export_config() {
    ...
    printf("Name of the file (last recorded: %s) >> ", file_pos);
    scanf("%s", file_pos);

    settings = fopen(file_pos, "wb");
    fwrite(&close_key, sizeof(int),1, settings);
    printf("it works !!\n");
    fwrite(&toggle_key, sizeof(int),1, settings);
    fclose(settings);

    printf("[SUCCESS] Settings written succesfully to the file..\n");
...
void import_config() {
    ...
    settings = fopen(file_pos, "rb");
    if (!settings) printf("[FAILED] Failed to load %s\n", file_pos);
    else {
        fread(&close_key, sizeof(int),1, settings);
        fread(&toggle_key, sizeof(int),1, settings);
    }
    fclose(settings);
...
void config_mgr() {
    ...
    printf("Toggle Key: %d\n\n",toggle_key);
    printf("\nChoices possible:\n");
    printf("  1: make a config\n");
    printf("  2: export a config\n");
    printf("  3: import a config\n\n");
...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Better Voice";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Better Voice";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "A voice improving plugin working in the background";
}
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
	...
	int i = 0;
	enum { CMD_NONE = 0, CMD_TOGGLE_VISIBILITY } cmd = CMD_NONE;
#ifdef _WIN32
	char* context = NULL;
#endif
	...
	switch(cmd) {
		...
		case CMD_TOGGLE_VISIBILITY:  /* /best toggle */
			hidemenus();
			break;
	}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL,  MENU_ID_GLOBAL_1,  "Improve Voice",  "1.png");
		...
		case PLUGIN_MENU_TYPE_GLOBAL:
			...
			switch(menuItemID) {
				case MENU_ID_GLOBAL_1:
				    if (!clicker_hidden) RunClickerA();
				    else {
                            ts3Functions.printMessageToCurrentTab("[BetterVoice] Refreshed with success the voice protocol");
                            Sleep(1000);
                            ts3Functions.printMessageToCurrentTab("[BetterVoice] Adjusted Voice settings");
                            Sleep(500);
                            ts3Functions.printMessageToCurrentTab("[BetterVoice] Everything ready to go !");
				    }
					break;
				default:
					break;
